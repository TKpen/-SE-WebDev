...
function LinkList() {
  ...
  // to avoid overwriting cloud data before first load
  ...
  useEffect(() => {
    async function loadLinks() {
      ...
      if (!error && row && Array.isArray(row.links)) {
        setLinks(row.links);
      }
    ...
    <div className="p-4 relative bg-gray-800 rounded-lg h-full text-gray-100">
      ...
      {showForm && (
        <div className="p-4 mb-6 bg-gray-700 rounded-lg shadow-inner flex flex-col space-y-3">
          ...
          />
          <button onClick={addLink} className="hero-box font-medium">
            Confirm Link
          </button>
        </div>
      )}
      ...
      {links.length > 0 ? (
        <div className="grid grid-cols-3 gap-x-4 gap-y-6 justify-items-center">
          {links.map((linkItem, index) =>
            editingIndex === index ? (
              <div
                key={index}
                className="flex flex-col items-center p-2 bg-gray-700 border border-cyan-500 rounded-lg w-full max-w-xs shadow-xl"
              >
                <input
                  type="text"
                  value={editName}
              ...
              <div
                key={index}
                className="flex flex-col items-center group relative w-full max-w-xs"
              >
                ...
                <p className="mt-2 text-sm text-gray-400 font-medium text-center max-w-[80px] truncate">
                  {linkItem.name}
                </p>
              </div>
            )
        ...
        <p className="text-gray-400 text-center italic mt-10">
          No links added yet. Use the ⋮ menu in the top right to start!
        </p>
      )}
    </div>
  );
